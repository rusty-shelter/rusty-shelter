...
impl CryptoParams {
    ...
    ///
    pub fn new(cipher: Cipher, ops_cost: u32, mem_cost: u32) -> Self {
        Self {
            cipher,
            ops_cost,
            mem_cost,
    ...
    #[inline]
    pub fn gen_secret_key() -> aead::SecretKey {
        aead::SecretKey::default()
    }

    pub fn hash_password(&self, password: &[u8], salt: &[u8]) -> aead::SecretKey {
        ...
        kdf::derive_key(&pass, &salt, self.ops_cost, self.mem_cost, 32).expect("kdf::derive_key should work")
    }

    #[inline]
    pub fn encrypt_with_key(&self, key: &aead::SecretKey, data: &[u8]) -> Vec<u8> {
        match self.cipher {
    ...
    #[inline]
    pub fn decrypt_with_key(&self, key: &aead::SecretKey, ciphertext: &[u8]) -> Vec<u8> {
        match self.cipher {
...
mod tests {
    ...
    #[test]
    fn enc_dec() {
        let crypto = CryptoParams::new(Cipher::XChaCha20Poly1305, 256, 3);
        const LEN: usize = 10;
